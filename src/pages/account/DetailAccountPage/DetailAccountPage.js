...
import UpdateLecturerProfile from "./UpdateLecturerProfile";
    ...
    return (
        <>
            ...
            <Tabs defaultActiveKey="1" type="card" size={"middle"}>
                ...
                {
                    (userRole < 2 || currentUser.uuid == uuid) ?
                        <Tabs.TabPane tab="Chỉnh sửa hồ sơ" key="2">
                            {
                                function () {
                                    ...
                                    if (role == 0 || (userRole === 1 && detailUser === 1)) {
...
const DetailAccountPage = () => {
    ...
    useEffect(() => {
        if(detailUser)
        setIsModerator(detailUser.account.role === 1);
    }, [detailUser])

    const onSetAdmin = (checked) => {

        if(checked) {
            axios.put(`/accounts/moderator/${uuid}`)
                .then(res => {
                    message.success("Đã phân quyền " + detailUser.fullname + " làm quản trị viên" )
                    setIsModerator(true);
                })
                .catch(e => {
            ...
            axios.put(`/accounts/moderator/unset/${uuid}`)
                .then(res => {
                    message.success("Đã hủy quyền quản trị viên của " + detailUser.fullname)
                    setIsModerator(false);
                })
                .catch(e => {
                    message.error("Đã có lỗi xảy ra")
